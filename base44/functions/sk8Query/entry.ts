...
// Logic is FROZEN. Edit only MCP_URL below. No secret required here.
// ============================================================================

// ---- Fill per deployment (non-secret) -------------------------------------
const MCP_URL = "https://<YOUR-SK8-URL>/api-gateway/v1/mcp";
...
async function mcpCall(token, sessionId, method, params) {
...
Deno.serve(async (req) => {
  try {
    ...
    let fresh = false;
    if (!sessionId) { sessionId = await mcpInit(sk8Token); fresh = true; }
    ...
    let result;
    try {
      result = await mcpCall(sk8Token, sessionId, method, params);
    } catch (e) {
      if (e.sessionStale && !fresh) {
        sessionId = await mcpInit(sk8Token);
        result = await mcpCall(sk8Token, sessionId, method, params);
      } else {
        throw e;
      }
